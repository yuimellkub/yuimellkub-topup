(function(){
  ...
  function install(){
    ...
    window.saveOrderToDemoAdmin=async function(){
      ...
      try{
        const compressed=await compressSlip(slip),reviewId=makeReviewId();window.currentOrderId=reviewId;const order=getLastOrderSafe();
        ...
        show('ok','✓ ส่งสลิปแล้ว • รอร้านตรวจสอบ');setTimeout(()=>watchReview(reviewId),60);setTimeout(()=>rewritePending(reviewId),150);setTimeout(()=>rewritePending(reviewId),500);return true;
