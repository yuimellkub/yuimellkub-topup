(function(){
  ...
  async function sendDiscordNotice(kind,id,draft){
    try{
      ...
      const r=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:'Yuimellkub Orders',allowed_mentions:{parse:[]},embeds:[{title,description:isOrder?'ชำระแล้ว • รอเติม':'มีสลิปใหม่ • รอตรวจสอบ',fields,timestamp:new Date().toISOString()}]})});
