(function(){
  ...
  function rewritePending(reviewId){
    ...
    while((n=w.nextNode())){if(!n.nodeValue)continue;n.nodeValue=n.nodeValue
      ...
      .replace('ส่งออเดอร์เรียบร้อยแล้ว ♡','ส่งสลิปเรียบร้อยแล้ว • รอร้านตรวจสอบ ♡')
      .replace('เลขออเดอร์ของคุณ','เลขอ้างอิงสลิป')
  ...
  function rewriteApproved(reviewId,orderId){
    ...
    while((n=w.nextNode())){if(!n.nodeValue)continue;n.nodeValue=n.nodeValue
      ...
      .replace('เลขอ้างอิงสลิป','เลขออเดอร์ของคุณ')
      .replace('ยังไม่มีเลขออเดอร์ • ร้านจะสร้างออเดอร์หลังยืนยันสลิปแล้ว','กรุณาบันทึกเลขออเดอร์นี้ไว้สำหรับติดตามสถานะ')
  ...
  function watchReview(reviewId){
    const db=getDb();if(!db)return;stopWatch();pendingReviewId=reviewId;rewritePending(reviewId);show('wait','กำลังรอร้านตรวจสอบสลิป • ยังไม่มีการสร้างออเดอร์');
    statusUnsub=db.collection('order_status').doc(reviewId).onSnapshot(snap=>{
      if(reviewId!==pendingReviewId||!snap.exists)return;
      const d=snap.data()||{};
      if(d.status==='สลิปไม่ผ่าน'||d.paymentStatus==='สลิปไม่ผ่าน'){
        rewritePending(reviewId);show('bad',d.slipReviewMessage||'ตรวจสอบสลิปไม่สำเร็จ กรุณาแนบสลิปที่ถูกต้องแล้วส่งใหม่อีกครั้ง');
      ...
      if(d.status==='ยืนยันแล้ว'&&d.paymentStatus==='ชำระแล้ว'&&d.approvedOrderId){
        rewriteApproved(reviewId,String(d.approvedOrderId));show('ok','✓ ร้านยืนยันสลิปแล้ว • สร้างออเดอร์ '+d.approvedOrderId+' เรียบร้อยแล้ว');
  ...
  function install(){
    ...
    window.saveOrderToDemoAdmin=async function(){
      const db=getDb();if(!db)return original.apply(this,arguments);if(!(await isManual(db)))return original.apply(this,arguments);
      stopWatch();const slip=document.getElementById('slipFile')?.files?.[0]||null;show('wait','กำลังส่งสลิปให้ร้านตรวจสอบ…');
      try{
        ...
        return true;
      }catch(e){console.error(e);show('bad','ส่งสลิปไม่สำเร็จ: '+(e?.message||'ไม่ทราบสาเหตุ'));return false;}
    };
  }
