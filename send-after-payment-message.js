(function(){
  ...
  function fixMessage(card){
    const nodes=[...card.querySelectorAll('div,p,span')].filter(el=>!el.children.length);
    for(const el of nodes){
      const t=(el.textContent||'').trim();
      if(t.includes('กรุณาเก็บเลขออเดอร์ไว้สำหรับติดตามสถานะ')&&t.includes('และรอร้านดำเนินการเติมสักครู่นะคะ')){
        el.textContent='กรุณาเก็บเลขออเดอร์ไว้สำหรับติดตามสถานะนะคะ ♡';
        continue;
      }
      if(t.includes('และรอร้านดำเนินการเติมสักครู่นะคะ'))el.remove();
      else if(t==='กรุณาเก็บเลขออเดอร์ไว้สำหรับติดตามสถานะ')el.textContent='กรุณาเก็บเลขออเดอร์ไว้สำหรับติดตามสถานะนะคะ ♡';
    }
  }
  function patch(){
    if(!isSend())return;
    ...
    fixMessage(card);
    if(!card.querySelector('#ymkSendAfterPaymentNotice')){
      const n=notice(),copy=[...card.querySelectorAll('button,a')].find(el=>/คัดลอกเลขออเดอร์/.test(el.textContent||''));
      if(copy)copy.insertAdjacentElement('beforebegin',n);else card.appendChild(n);
    }
  }
  let busy=false;function schedule(){if(busy)return;busy=true;setTimeout(()=>{busy=false;patch();},0);}
  patch();new MutationObserver(schedule).observe(document.body,{childList:true,subtree:true,characterData:true});
})();
