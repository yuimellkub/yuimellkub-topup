(function(){
  ...
  if(document.body){updateSlipCopy();new MutationObserver(()=>updateSlipCopy()).observe(document.body,{childList:true,subtree:true});startModeListener();}
  else document.addEventListener('DOMContentLoaded',()=>{updateSlipCopy();new MutationObserver(()=>updateSlipCopy()).observe(document.body,{childList:true,subtree:true});startModeListener();},{once:true});
  ...
  async function getSlipMode(db){
    try{const snap=await db.collection('products').doc(SETTINGS_ID).get();const mode=snap.exists?snap.data()?.slipVerificationMode:'auto';setSlipMode(mode==='manual'?'manual':'auto');}
    catch(e){console.warn('read slip mode failed, using auto',e);setSlipMode('auto');}
  ...
  async function verifySlip(file,expectedAmount){
    ...
    const form=new FormData();form.append('image',file,file.name||'slip.jpg');
    let response;try{response=await fetch(EASYSLIP_WORKER_URL,{method:'POST',body:form});}catch(error){throw systemUnavailable('เชื่อมระบบตรวจสลิปไม่ได้ ออเดอร์จะให้ร้านตรวจสอบแทน','NETWORK_ERROR');}
    ...
    if(data.isDuplicate)throw new Error('สลิปนี้ถูกใช้ไปแล้ว กรุณาใช้สลิปใหม่');
    const slipDate=data.rawSlip?.date||'',slipTime=Date.parse(slipDate);
    if(!slipDate||Number.isNaN(slipTime))throw new Error('ไม่พบวันและเวลาของรายการในสลิป กรุณาใช้สลิปใหม่');
    const slipAge=Date.now()-slipTime;
    if(slipAge>MAX_SLIP_AGE_MS)throw new Error('สลิปนี้เก่าเกิน 30 นาที กรุณาใช้สลิปจากการชำระเงินครั้งล่าสุด');
    if(slipAge< -FUTURE_TOLERANCE_MS)throw new Error('วันหรือเวลาในสลิปไม่ถูกต้อง กรุณาตรวจสอบเวลาในอุปกรณ์แล้วลองใหม่');
    const amount=Number(data.amountInSlip??data.rawSlip?.amount?.amount),expected=parseMoney(expectedAmount);
    if(expected>0&&Number.isFinite(amount)&&Math.abs(amount-expected)>0.01)throw new Error('ยอดในสลิปไม่ตรงกับยอดออเดอร์ (สลิป '+amount.toFixed(2)+' บาท / ออเดอร์ '+expected.toFixed(2)+' บาท)');
    return {verified:true,amount:Number.isFinite(amount)?amount:null,transRef:data.rawSlip?.transRef||'',slipDate,isDuplicate:false};
  }
  ...
  function compressSlip(file){
    return new Promise((resolve,reject)=>{
      ...
      const reader=new FileReader(),image=new Image();reader.onerror=()=>reject(new Error('อ่านรูปสลิปไม่สำเร็จ'));image.onerror=()=>reject(new Error('เปิดรูปสลิปไม่สำเร็จ'));reader.onload=()=>{image.src=String(reader.result||'');};
      image.onload=()=>{try{const maxSide=1400,scale=Math.min(1,maxSide/Math.max(image.naturalWidth,image.naturalHeight)),width=Math.max(1,Math.round(image.naturalWidth*scale)),height=Math.max(1,Math.round(image.naturalHeight*scale)),canvas=document.createElement('canvas');canvas.width=width;canvas.height=height;const ctx=canvas.getContext('2d',{alpha:false});ctx.fillStyle='#fff';ctx.fillRect(0,0,width,height);ctx.drawImage(image,0,0,width,height);let quality=.78,data=canvas.toDataURL('image/jpeg',quality);while(data.length>MAX_DATA_URL_BYTES&&quality>.42){quality-=.08;data=canvas.toDataURL('image/jpeg',quality);}if(data.length>MAX_DATA_URL_BYTES)throw new Error('รูปสลิปยังมีขนาดใหญ่เกินไป กรุณาครอปหรือใช้ภาพที่เล็กลง');resolve({data,width,height,bytes:data.length});}catch(error){reject(error)}};
  ...
  saveOrderToDemoAdmin=async function(){
    ...
    const order={id:currentOrderId||makeOrderId(),createdAt:new Date().toISOString(),item:lastOrder?.item||'',pack:lastOrder?.pack||'',price:lastOrder?.price||'',paymentMethod:getPaymentMethod(),uid,server,name,paymentStatus:'รอตรวจสอบการชำระเงิน',shopStatus:'รอเติม',orderReady:false};
    ...
    if(firebaseReady()){
      try{
        ...
        if(slip&&selectedMode==='auto'){
          ...
          catch(e){if(!e?.slipSystemUnavailable)throw e;manualReason=e.slipSystemCode||'SYSTEM_UNAVAILABLE';order.paymentStatus='รอร้านตรวจสอบการชำระเงิน';order.slipVerified=false;order.slipVerificationMode='manual-fallback';order.slipFallbackReason=manualReason;order.orderReady=false;st.textContent='ระบบตรวจสลิปอัตโนมัติไม่พร้อมใช้งาน • จะส่งสลิปให้ร้านตรวจสอบก่อน';}
        }else if(slip){order.paymentStatus='รอร้านตรวจสอบการชำระเงิน';order.slipVerified=false;order.slipVerificationMode='manual';order.orderReady=false;st.textContent='โหมดร้านตรวจเอง • กำลังส่งสลิปให้ร้านตรวจสอบ…';}

        if(!manualReason)st.textContent=verification?'ตรวจสลิปผ่านแล้ว กำลังส่งออเดอร์…':(slip?'กำลังบันทึกรูปสลิปเพื่อรอร้านตรวจสอบ…':'กำลังส่งออเดอร์เข้าระบบออนไลน์…');
        ...
        if(compressed.data){const slipRef=db.collection('order_slips').doc(order.id);batch.set(slipRef,{orderId:order.id,imageData:compressed.data,mimeType:'image/jpeg',width:compressed.width,height:compressed.height,bytes:compressed.bytes,verified:!!verification,verifiedAmount:verification?.amount??null,transRef:verification?.transRef||'',verificationMode:order.slipVerificationMode||selectedMode,fallbackReason:manualReason,createdAt:firebase.firestore.FieldValue.serverTimestamp()});}
        await batch.commit();

        if(order.orderReady){
          try{await db.collection('order_status').doc(order.id).set({status:order.shopStatus,paymentStatus:order.paymentStatus,updatedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});}catch(e){console.warn('public status create skipped',e);}
          try{localStorage.setItem('ymk_order_status_'+order.id,order.shopStatus)}catch(e){}
        }

        st.className='verify-status ok';
        st.textContent=verification?'✓ ตรวจสลิปผ่านและสร้างออเดอร์แล้ว: '+order.id:slip?'✓ ส่งสลิปแล้ว • รอร้านยืนยันก่อนสร้างออเดอร์: '+order.id:'✓ ส่งออเดอร์แล้ว: '+order.id;
        return true;
